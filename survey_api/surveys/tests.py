...
from .models import Category, Survey, Question, Answer, UserResponse
...
class SurveyTests(TestCase):
    def setUp(self):
        ...
        self.choice_question = Question.objects.create(
            survey=self.survey, text="What is your favorite color?", type="choice", order=2
        )
    ...
    def test_add_response_text_question(self):
        ...
        response = self.client.post(
            f'api/v1/surveys/{self.survey.id}/submit-response/',
            {
                ...
                "text_response": "This is an answer"
            },
            format="json"
        )
    ...
    def test_export_responses_csv(self):
        ...
        response = self.client.get(f"/surveys/{self.survey.id}/export-responses/?format=csv")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
